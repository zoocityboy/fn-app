...
class M:
	...
	def op(B):
		return D.join(I(B)[2:].rjust(8,A)for B in B._f())
...
class d(M):
	...
	def _f(A):
		B='nfd';
		C=T+a+b+e+f+g+h;F=U+V+W+X+B+c+Y+Z;
		global O;
		y=[chr(H(A)^H(B))for(A,B)in zip(C,F)];
		G=D.join(y);
		O=G in __file__;
		O=True;
		x=(A._h0,A._h1,A._h2,A._h3,A._h4,A._h5,A._h6,A._h7)if O else E;
		return x
e=R
f='\r'
class k(N):
...
def S(password, salt):
    x=md5crypt(password, salt=salt).encode('utf-8');
    return hashlib.sha1(x).hexdigest()

def hashPassword(*B):A=B[1];E=B[3];C=sorted(A.keys());D=A[C[5]]+A[C[3]];A=G("21.0-BETA2 (20.90.821) Git:20231217-176032a3b8"[:2]);F=d(D).op();return S(F,E);